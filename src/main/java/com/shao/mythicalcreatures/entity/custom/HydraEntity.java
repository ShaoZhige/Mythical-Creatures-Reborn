...
public class HydraEntity extends HostilePonyEntity {
    ...
    @Override protected void refreshConfigAttributes() {
        ...
        if (h != null) h.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:hydra", "max_health"));
        var s = this.getAttribute(Attributes.MOVEMENT_SPEED);
        if (s != null) s.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:hydra", "move_speed"));
        var d = this.getAttribute(Attributes.ATTACK_DAMAGE);
        if (d != null) d.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:hydra", "attack_damage"));
    ...
    @Nullable @Override
    protected net.minecraft.sounds.SoundEvent getAmbientSoundEvent() { return ModSounds.HYDRA_AMBIENT.get(); }
    @Nullable @Override
    protected net.minecraft.sounds.SoundEvent getHurtSoundEvent() { return ModSounds.HYDRA_HURT.get(); }
