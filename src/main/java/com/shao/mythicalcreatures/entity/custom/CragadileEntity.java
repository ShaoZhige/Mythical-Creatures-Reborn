...
public class CragadileEntity extends HostilePonyEntity {
    ...
    @Override protected void refreshConfigAttributes() {
        ...
        if (h != null) h.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:cragadile", "max_health"));
        var s = this.getAttribute(Attributes.MOVEMENT_SPEED);
        if (s != null) s.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:cragadile", "move_speed"));
        var d = this.getAttribute(Attributes.ATTACK_DAMAGE);
        if (d != null) d.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:cragadile", "attack_damage"));
