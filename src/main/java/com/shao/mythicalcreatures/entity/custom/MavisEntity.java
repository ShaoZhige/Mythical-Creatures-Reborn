...
public class MavisEntity extends PonyEntity {
    ...
    @Override protected void refreshConfigAttributes() {
        ...
        if (h != null) h.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:mavis", "max_health"));
        var s = this.getAttribute(Attributes.MOVEMENT_SPEED);
        if (s != null) s.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:mavis", "move_speed"));
        var d = this.getAttribute(Attributes.ATTACK_DAMAGE);
        if (d != null) d.setBaseValue(MythicalConfig.DATA.entityAttr("mythicalcreatures:mavis", "attack_damage"));
